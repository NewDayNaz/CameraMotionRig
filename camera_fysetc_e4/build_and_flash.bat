...
:ota_upload
    ...
    echo     # Parse JSON response - handle whitespace in JSON >> "%PS_SCRIPT%"
    echo     $responseText = $responseText -replace '\s+', ' ' >> "%PS_SCRIPT%"
    echo     if ($responseText -match '"status"\s*:\s*"ok"') { >> "%PS_SCRIPT%"
