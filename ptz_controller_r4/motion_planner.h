...
// Expo curve factor (1.6-2.4 typical)
#define JOYSTICK_EXPO 1.8f
...
typedef struct {
    ...
    float manual_target_vel[NUM_AXES]; // Target velocity from joystick
} motion_planner_t;
 ...
 * @param planner Planner structure
 * @param velocities Target velocities for each axis (steps/sec, normalized -1 to 1)
 */
